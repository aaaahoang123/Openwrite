...
def test_chat_adapter_config_from_launcher_environment(monkeypatch):
    monkeypatch.setenv("AGENT_CHAT_SESSION_ID", "10")
    monkeypatch.setenv("AGENT_CHAT_TURN_ID", "20")
    monkeypatch.setenv("AGENT_PROJECT_ID", "30")
    monkeypatch.setenv("AGENT_STORY_ID", "40")
    ...
    monkeypatch.setenv("AGENT_SOURCE_BRANCH", "main")
    monkeypatch.setenv("AGENT_SOURCE_COMMIT_SHA", "abcdef123456")
    monkeypatch.setenv("AGENT_MCP_ENDPOINT", "http://localhost:8000")
    monkeypatch.setenv("AGENT_MCP_TOKEN", "secret")
    monkeypatch.setenv("AGENT_GIT_CLONE_URL", "https://github.com/test/repo")
    monkeypatch.setenv("AGENT_GIT_USERNAME", "gituser")
    monkeypatch.setenv("AGENT_GIT_PASSWORD", "gitpass")
    monkeypatch.setenv("AGENT_LLM_PROVIDER", "openai")
    monkeypatch.setenv("AGENT_LLM_MODEL", "gpt-4")
    monkeypatch.setenv("AGENT_LLM_API_KEY", "sk-123")
    monkeypatch.setenv("AGENT_LLM_BASE_URL", "https://api.openai.com/v1")
    monkeypatch.setenv("OPENWRITE_LANGUAGE", "en")
    monkeypatch.setenv("OPENWRITE_WORKSPACE", "/workspace")
...
def test_chat_adapter_config_missing_turn_id(monkeypatch):
    ...
    with pytest.raises(
        ConfigError, match="Missing required environment variable: AGENT_CHAT_TURN_ID"
    ):
